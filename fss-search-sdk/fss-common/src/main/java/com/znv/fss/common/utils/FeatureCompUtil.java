...
public class FeatureCompUtil {
    protected static final Log LOG = LogFactory.getLog(FeatureCompUtil.class);
...
//    private static float[] dst_points = {0.0f, 0.4f, 0.5f, 0.6f, 0.7f, 0.85f, 0.95f, 1.0f,};
    ...
    // points read from HDFS
    public static void setFeaturePoints(Map<String, float[]> featuresPoints) {
        if (featuresPoints.containsKey(VConstants.SENSETIME_FEATURE_SRC) && null != featuresPoints.get(VConstants.SENSETIME_FEATURE_SRC) && featuresPoints.get(VConstants.SENSETIME_FEATURE_SRC).length > 0) {
            src_points = featuresPoints.get(VConstants.SENSETIME_FEATURE_SRC);
        }
        if (featuresPoints.containsKey(VConstants.SENSETIME_FEATURE_DST) && null != featuresPoints.get(VConstants.SENSETIME_FEATURE_DST) && featuresPoints.get(VConstants.SENSETIME_FEATURE_DST).length > 0) {
            dst_points = featuresPoints.get(VConstants.SENSETIME_FEATURE_DST);
        }
    }
