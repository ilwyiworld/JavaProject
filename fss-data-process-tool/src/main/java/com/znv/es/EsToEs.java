        ...
        try {
            ...
            do {
                ...
                for (SearchHit hit : scrollResp.getHits().getHits()) {
                    //Handle the hit...
                    Map<String, Object> source = hit.getSource();//hit.getSource()
                    bulkRequestBuilder.add(indexRequestBuilder.setSource(jsonBuilder().map(source)));
                }
                bulkRequestBuilder.execute().actionGet();
                scrollResp = sourceClient.prepareSearchScroll(scrollResp.getScrollId()).setScroll(new TimeValue(60000000)).execute().actionGet();
            }
