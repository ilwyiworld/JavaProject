...
public class RouteController implements RouteApi {
    ...
    public BaseResponse<NULLBody> groupRoute(@RequestBody ChatReqVO groupReqVO) throws Exception {
        ...
        for (Map.Entry<Long, ServerResVO> cimServerResVOEntry : serverResVOMap.entrySet()) {
            Long userId = cimServerResVOEntry.getKey();
            ServerResVO cimServerResVO = cimServerResVOEntry.getValue();
            if (userId.equals(groupReqVO.getUserId())){
                //过滤掉自己
                UserInfo cimUserInfo = userInfoCacheService.loadUserInfoByUserId(groupReqVO.getUserId());
    ...
    public BaseResponse<NULLBody> p2pRoute(@RequestBody P2PReqVO p2pRequest) throws Exception {
        ...
        try {
            //获取接收消息用户的路由信息
            ServerResVO cimServerResVO = accountService.loadRouteRelatedByUserId(p2pRequest.getReceiveUserId());
    ...
    public BaseResponse<Set<UserInfo>> onlineUser() throws Exception {
        ...
        Set<UserInfo> cimUserInfos = userInfoCacheService.onlineUser();
